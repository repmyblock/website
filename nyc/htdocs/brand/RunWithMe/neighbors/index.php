<?php
	...
	if ( ! empty ($_POST)) {
		if ($_POST["checkoneyes"] == "Prepare the petition") {
			...
			// $StringULR . ""
			header("Location: ../prepwitness/?k=" . EncryptURL($StringURL));
			exit();
		}
	...
	if ( ! empty ($_GET["k"])) {
		...
		$r = new runwithme();	
		$result = $r->FindNeibors($URIEncryptedString["NYSID"], $URIEncryptedString["HseNbr"], 
														  $URIEncryptedString["FracAdd"], $URIEncryptedString["Apt"], 
														  $URIEncryptedString["PreSt"], $URIEncryptedString["St"], 
														  $URIEncryptedString["PostSt"], $URIEncryptedString["City"], 
													    $URIEncryptedString["Zip"], "Raw_Voter_" . $DatedFiles);	
