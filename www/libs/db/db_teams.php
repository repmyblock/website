...
class Teams extends RepMyBlock {
  ...
  function FindCampaignFromCode ($TeamCode) {
	 	return $this->_return_simple(
	 		"SELECT * FROM Team WHERE Team_AccessCode = :TeamCode", 
	 		["TeamCode" => $TeamCode]
	 	);
	}
	...
	function ListTeamsWithMembers($SystemUserID) {		
		return $this->_return_multiple(
				"SELECT *, TeamMember.SystemUser_ID AS SystemIDFromTeam FROM Team " . 
				"LEFT JOIN TeamMember ON (TeamMember.Team_ID = Team.Team_ID) " . 
				"WHERE " . 
				"(TeamMember.SystemUser_ID = :SystemUser && TeamMember_RemovedBy is null) " . 
				"OR (Team_Public = 'public')", 
	...
	function ListActiveTeam($SystemUserID, $Active = 'yes') {		
		return $this->_return_multiple(
				"SELECT *, TeamMember.SystemUser_ID AS SystemIDFromTeam FROM Team " . 
				"LEFT JOIN TeamMember ON (TeamMember.Team_ID = Team.Team_ID " . 
				"AND TeamMember_Active = 'yes') " . 
				"WHERE (TeamMember.SystemUser_ID = :SystemUser AND " . 
				"(TeamMember_Active = :Active OR TeamMember_Active = 'pending')) " .
				" OR (Team_Public = 'public' AND Team_Active = :Active)", 
				["SystemUser" => $SystemUserID, "Active" => $Active]
		);
	}
	...
	function SearchUsersForMyTeam($SystemUser_ID) {
		return $this->_return_multiple(
				...
				"(Candidate.Candidate_UniqStateVoterID = SystemUser.Voters_UniqStateVoterID AND " . 
				"Candidate.Team_ID = Team.Team_ID) " .
				"LEFT JOIN CandidateGroup ON (Candidate.Candidate_ID = CandidateGroup.Candidate_ID) " .
				"WHERE Team.SystemUser_ID = :SystemID AND " . 
	...
	function ReturnMemberFromTeam($TeamMemberID) {
		return $this->_return_simple(
				"SELECT *, TeamMember.SystemUser_ID as TeamSystemUser_ID FROM TeamMember " .
	...
	function ListMyTeam($SystemUser_ID) {
		return $this->_return_multiple(
				"SELECT * FROM Team " .  
				"LEFT JOIN TeamMember ON (TeamMember.Team_ID = Team.Team_ID) " .
				"WHERE (TeamMember.SystemUser_ID = :SystemUser AND TeamMember.TeamMember_Privs > :MaxPrivs " . 
				"AND TeamMember_RemovedDate IS NULL) OR Team.SystemUser_ID = :SystemUser",
				["SystemUser" => $SystemUser_ID, "MaxPrivs" => 16]
		);
	}
	
	function ListBannedMembers($Team_ID) {
		return $this->_return_multiple(
				"SELECT *, SystemUser.SystemUser_FirstName AS TeamFirst, SystemUser.SystemUser_LastName AS LastLast," . 
	...
	function ListUnsignedMembers($TeamID) {
		return $this->_return_multiple(
				"SELECT *, SystemUserEmail.SystemUserEmail_MailCode as MailCode_First, " .
	...
	function CheckTeamExist($TeamName, $TeamAccessCode) {
		return $this->_return_simple(
				...
				["TeamName" => $TeamName, "TeamAccessCode" => $TeamAccessCode]
		);	
	}
  ...
  function FindPositions($ElectionsID, $Party, $Type, $Value, $TeamID = NULL) {
  	return $this->_return_multiple(
  			...
  			"EPC1.ElectionsPartyCall_ConversionValue = :Value AND Candidate_ID IS NOT NULL AND " . 
  			"CandidateElection.Elections_ID IS NOT NULL",
				["Elections_ID" => $ElectionsID, "Party" => $Party, "Type" => $Type , "Value" => $Value]
		);
  }
