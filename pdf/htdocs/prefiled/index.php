...
switch ($Variable) {
	...
	case 'prefiled':
		$ResHouseNumber = "640";
		$ResStreetName = "Riverside Drive West";
		$ResZip = "10031";
		$CandidatePetitionSet_ID = 243;
		
		# CREATE INDEX Raw_Voter_20200203_ResHouseNumber_ResStreetName_Zip_IDX ON Raw_Voter_20200203 (Raw_Voter_ResHouseNumber, Raw_Voter_ResStreetName, Raw_Voter_ResZip);
		...
		if ( ! empty ($result)) {
			...
			$pdf->BarCode = "M" . $result[0]["CandidatePetitionSet_ID"];
			$ElectionDate = PrintShortDate($result[0]["Elections_Date"]);
		}
		
		
		$Addresses = $r->GetListAddresses($DatedFiles, $ResHouseNumber, $ResStreetName, $ResZip);
		$counter = 0;
		if ( ! empty ($Addresses)) {
			foreach ($Addresses as $var) {
				if ( ! empty ($var)) {
					if ( $var["Raw_Voter_Status"] == "ACTIVE" && $var["Raw_Voter_EnrollPolParty"] = "DEM") {
											
						$apt = preg_replace('/APT/', '', $var["Raw_Voter_ResApartment"]);
						$apt = preg_replace('/-/', '', $apt);
						$apt = preg_replace('/\s+/', '', $apt);
						
						$FoundApt = 0;
						for ($i = 0; $i < count(	$AptSorter[$var["Raw_Voter_ResZip"]][$var["Raw_Voter_ResStreetName"]][$var["Raw_Voter_ResHouseNumber"]] ); $i++) {
							if ( $AptSorter[$var["Raw_Voter_ResZip"]][$var["Raw_Voter_ResStreetName"]][$var["Raw_Voter_ResHouseNumber"]][$i] == $apt) {
								$FoundApt = 1;
							}						
						}
						if ( $FoundApt == 0) { $AptSorter[$var["Raw_Voter_ResZip"]][$var["Raw_Voter_ResStreetName"]][$var["Raw_Voter_ResHouseNumber"]][] = $apt; }
						
						$NewOrder[$var["Raw_Voter_ResZip"]][$var["Raw_Voter_ResStreetName"]][$var["Raw_Voter_ResHouseNumber"]]
											[$apt][$var["Raw_Voter_ID"]]["FullName"] =  $r->DB_ReturnFullName($var); 
						$NewOrder[$var["Raw_Voter_ResZip"]][$var["Raw_Voter_ResStreetName"]][$var["Raw_Voter_ResHouseNumber"]]
											[$apt][$var["Raw_Voter_ID"]]["AddressLine1"] =  $r->DB_ReturnAddressLine1($var);
						if ( ! empty ($apt) ) { 
							$NewOrder[$var["Raw_Voter_ResZip"]][$var["Raw_Voter_ResStreetName"]][$var["Raw_Voter_ResHouseNumber"]]
												[$apt][$var["Raw_Voter_ID"]]["AddressLine1"] .= " - Apt " . $apt; 
						} 					
						$NewOrder[$var["Raw_Voter_ResZip"]][$var["Raw_Voter_ResStreetName"]][$var["Raw_Voter_ResHouseNumber"]]
											[$apt][$var["Raw_Voter_ID"]]["AddressLine2"] =  $r->DB_ReturnAddressLine2($var);
		...
		// This is to order the pages;
		if ( ! empty ($NewOrder)) {
			foreach ($NewOrder as $Zipcodes => $ArrAddress) {
				foreach ($ArrAddress as $Add => $ArrNumber) {
					foreach ($ArrNumber as $Number => $ArrApt) {
						if ( ! empty ($Number)) {
					
							natcasesort ($AptSorter[$Zipcodes][$Add][$Number]);
							foreach ($AptSorter[$Zipcodes][$Add][$Number] as $AptNumber) {
								foreach ($ArrApt[$AptNumber] as $Person) {
									$County[$counter] = "NY";
									$Name[$counter] =  $Person["FullName"];
									$Address[$counter] = $Person["AddressLine1"] . "\n" . $Person["AddressLine2"];
									$counter++;
								}
								$NewPage[$counter] = "yes";
							}
		...
		$Petition_FileName = $ResHouseNumber . "_" . $ResStreetName . "_" . $ResZip;
		break;
...
for ($i = 0; $i < $TotalCountName; $i++) {
	...
	$pdf->Cell(38, 0, $Counter . ". March 16, ". date("Y"), 0, 0, 'L', 0);
	
	$pdf->SetXY(121, $YLocation - 7);
	$pdf->MultiCell(73, 4, $Address[$i], 0, 'L', 0);
	
	$pdf->SetXY(195, $YLocation - 4.5);
