 ...
 **/
	  ...
	  class FPDF_Merge{
        ...
        // ===== THIS IS MY MODIFICATION WITH STR
        ...
        // ##### String object
         private function getStrObject($string, $xref, $index, $includeSubObject = false){
                ...
                case self::TYPE_DICTIONARY:
                    ...
                    foreach($item[1] as $key => $val){
                        if ( ($val[0] == self::TYPE_REFERENCE) || 
                             ($val[0] == self::TYPE_ARRAY) ||
                             ($val[0] == self::TYPE_DICTIONARY) ){
                            $r[$key] = $this->_resolveStrValues($string, $xref, $val);
                        } else {
                            $r[$key] = $val;
                        }
                    }
        ...
        private function getStrResources($string, $xref, $page){
            ...
            if (isset($page[1]['/Resources'])){
                ...
                return $this->getStrResources($string, $xref, $page[1]['/Parent']);
            }
            return null;
        }
        ...
        private function getStrContent($string, $xref, $page){
            ...
            $stream = '';
            if (isset($page[1]['/Contents'])){
                $stream = $this->_getStrContent($string, $xref, $page[1]['/Contents']);
            }
            return $stream;
        }
        ...
        private function _getStrContent($string, $xref, $content){
            ...
            if ($content[0] === self::TYPE_REFERENCE){
                ...
                $stream .= $this->getStrStream($string, $xref, $item);
            }
            return $stream;
        }
        ...
        private function getStrCompression($string, $xref, $item){
            if ($item[0] === self::TYPE_TOKEN){
                ...
                $r = array();
                foreach($item[1] as $sub){
                    $r = array_merge($r, $this->getStrCompression($string, $xref, $sub));
                }
                return $r;
            } else if ($item[0] === self::TYPE_REFERENCE){
                return $this->getStrCompression($string, $xref, $this->getStrObject($string, $xref, $item[1]));
            }
            return array();
        }
        ...
        public function add_string( $string ){
            ...
            foreach($pages['/Kids'][1] as $kid){
                ...
                $resources = $this->getStrResources($string, $xref, $kid);
                $resources = $resources[1][1];
                
                $content = $this->getStrContent($string, $xref, $kid);
                    ...
                    if (preg_match_all("`(/F[0-9]+)\s+-?[0-9\.]+\s+Tf`", $content, $matches, PREG_OFFSET_CAPTURE)){
                        ...
                        for($i=0; $i<$cnt; $i++){
                            ...
                            if (!isset($newFonts[$name])){
                                $object = $this->getStrObject($string, $xref, $resources['/Font'][1][$name][1], true);
                    ...
                    if (preg_match_all("`(/[^%\[\]<>\(\)\r\n\t/]+) Do`", $content, $matches, PREG_OFFSET_CAPTURE)){
                        ...
                        foreach($matches[1] as $m){
                            ...
                            if (!isset($newXObjects[$name])){
                                $object = $this->getStrObject($string, $xref, $resources['/XObject'][1][$name][1], true);
