<?php
//  $imgtoshow = "/brand/RunWithMe/RunWithMe.png";
  $HeaderTwitter = 1;
  $HeaderTwitterPicLink = "https://www.repmyblock.org/images/training/ZoomWithPaperboy.png";
  $HeaderTwitterDesc = "Learn how to run for office with Paperboy Love Prince.";   
  $HeaderTwitterTitle = "Paperboy Love Prince interview every politician.";   
  
  $HeaderOGImage = $HeaderTwitterPicLink;
  $HeaderOGDescription = $HeaderTwitterDesc;
  $HeaderOGImageWidth = "450";
  $HeaderOGImageHeight = "253";
  
  $VideoOnHtml = true;
    
  require_once $_SERVER["DOCUMENT_ROOT"] . "/../libs/common/verif_sec.php";  
  include $_SERVER["DOCUMENT_ROOT"] . "/common/headers.php"; 
  if ( $MobileDisplay == true ) { $TypeEmail = "email"; $TypeUsername = "username";
  } else { $TypeEmail = "text"; $TypeUsername = "text"; }
?>
    <DIV class="main">
      <DIV class="right f80">Training with Paperboy</DIV>
      <h4><P class="grep ">Training with Paperboy Love Prince</P></h4>

      <P>
        Paperboy Love Prince has ran for office continously from the smaller
        offices in the land such as County Committee all the way to President of the United States.
      </P>

      <P>
        Paperboy Love Prince discovered early on that the whole process was bias and rigged and
        participated in being part of the experiment of running and documenting
        the process each time.
      </P>

      <P>
        These episodes are made available to give you an idea of what to expect.
      </P>

      <P class="f40">
        <B>Episode #1:</B> Sal Albanese explains the corruption of the New York system.
      </P>
        
      <DIV class="videowrapper">
        <iframe src="https://www.youtube.com/embed/UwyfucMSzDY" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
      </DIV>
    
      <P class="f40">
        <B>Episode #2:</B> Badrun Khan explains the gruling work that goes into petitioning.
      </P>

      <DIV class="videowrapper">
        <iframe src="https://www.youtube.com/embed/lc_sgWGxZWo" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
      </DIV>
    
      <P class="f40">
        <B>Episode #3:</B> Ben Yee explains the work of the Activist.
      </P>

      <DIV class="videowrapper">
        <iframe src="https://www.youtube.com/embed/NU1hVBaXTNo" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
      </DIV>

      <P class="f40">
        <B>Episode #4:</B> Vittoria Fariello explain the expectation of being elected as a Party Official.
      </P>

      <DIV class="videowrapper">
        <iframe src="https://www.youtube.com/embed/GsQ7YtaunCE" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
      </DIV>

      <P class="f40">
        <B>Episode #5:</B> Jared Rich goes over the inportance of the access of the local press in being considered a viable candidate.
      </P>

      <DIV class="videowrapper">
        <iframe src="https://www.youtube.com/embed/AO7iBH0xgkA" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
      </DIV>

      <p class="f40">
        <B>Episode #6:</B> Dr. Devi Nampiaparampil, Republican candidate for Public Advocate, dive into the road blocks setup by the Campaign Finance Fund.
      </P>

      <DIV class="videowrapper">
        <iframe src="https://www.youtube.com/embed/Q7sQCZtM0bY" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
      </DIV>
        
      <P class="f40">
        Rep My Block is provided Free of Charge to any candidate that wishes to integrate 
        it's services.
      </P> 
    </DIV>    
<?php include $_SERVER["DOCUMENT_ROOT"] . "/common/footer.php"; ?>